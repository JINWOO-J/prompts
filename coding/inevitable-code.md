...
> 모든 설계 선택이 유일하게 합리적인 옵션처럼 느껴지는 코드를 작성하는 철학.

---

## 핵심 원칙

- 단순한 인터페이스로 내부 복잡성을 숨겨라 — 외부 인지 부하 제거가 목적
- 사용자에게 강제하는 API 선택을 줄여라 — JavaScript의 자연스러운 패턴 활용
- 기존 멘탈 모델을 활용하는 이름과 패턴을 선택하라 — 암기가 아닌 인식
- 클래스보다 함수, 상속보다 합성을 선호하라
- TypeScript 타입 시스템으로 실수를 방지하되, 불필요한 의식(ceremony)은 만들지 마라
- 과도한 추상화, 설정 폭발, 조기 일반화, 불필요한 서비스 레이어를 제거하라
- 코드를 내보내기 전 자문: 가능한 한 단순한가? 자연스러운가? 실제 문제를 해결하는가?

## 상세

Giselle AI 팀의 ts-coder 에이전트가 따르는 코딩 철학.
개발자가 코드를 만났을 때 "당연히 이렇게 되어야지. 다른 방법이 있겠어?"라고 느끼게 만드는 것이 목표다.

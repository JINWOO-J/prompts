...
source: '[Scoutflo/Scoutflo-SRE-Playbooks](https://github.com/Scoutflo/Scoutflo-SRE-Playbooks/blob/master/Sentry%20Playbooks/01-Error-Tracking/DatabaseConnectionError-ConnectionRefused-Database-Error-application.md)'
...
# DatabaseConnectionError-ConnectionRefused-Database-Error-application — 데이터베이스 연결 거부 에러

## 의미

데이터베이스 연결 에러가 발생합니다 (Sentry 에러 이슈 트리거, 예외 유형 DatabaseConnectionError). 데이터베이스 서버 연결이 거부되어 데이터베이스 서버에 연결 시도 시 데이터베이스 작업이 실패합니다. Sentry Issue Details 페이지에 "database connection refused", "cannot connect to database" 패턴의 에러 메시지가 표시되며, 스택 트레이스는 데이터베이스 연결 레이어(sentry/db/postgres/base.py 등)를 가리키고, 에러 레벨은 Error 또는 Fatal 심각도를 나타냅니다. 이는 데이터베이스 레이어에 영향을 미치며, 일반적으로 데이터베이스 서비스 다운, 네트워크 연결 문제, 또는 데이터베이스 설정 에러로 인한 데이터베이스 연결 문제를 나타냅니다.

## 영향

Sentry 에러 이슈 알림이 발생하고, 데이터베이스 연결이 실패하며, 데이터베이스 작업이 중단됩니다. 애플리케이션이 데이터베이스 에러를 반환하고 사용자에게 영향을 미칩니다. 데이터베이스 사용 불가로 인해 애플리케이션이 완전히 실패하고, 영향받는 사용자 수가 증가하며, 에러 카운트가 지속적으로 증가합니다. Sentry 대시보드에서 High 우선순위로 표시되고, 애플리케이션 기능이 완전히 저하됩니다. 모든 데이터베이스 의존 작업이 실패합니다.

## 플레이북

1. 이슈 유형(`metadata.type`), 우선순위(`priority`), 영향받는 사용자 수(`userCount`), 에러 메시지(`metadata.value`)를 포함한 이슈 상세 정보를 확인하여 "database connection refused" 또는 "cannot connect to database" 패턴의 DatabaseConnectionError를 검증합니다.

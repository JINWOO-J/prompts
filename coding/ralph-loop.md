...
> 스펙(테스트)을 먼저 작성하고, AI 에이전트가 모든 테스트를 통과할 때까지 자율적으로 반복하게 하는 방법론.

---

## 핵심 원칙

- 스펙(테스트)을 먼저 작성하고 에이전트에게 통과를 목표로 지시하라
- 테스트가 불완전하면 "테스트만 통과하는" 나쁜 코드가 생긴다 — 포괄적인 테스트 스위트가 전제 조건이다
- 무한 루프 방지를 위해 최대 반복 횟수를 설정하라
- 보안 관련 코드에는 부적합하다 — 수동 리뷰가 필수
- 테스트 실행 명령어를 CLAUDE.md에 명시하라
- 에이전트가 코드를 생성한 후 사람의 코드 리뷰와 부수효과 확인이 필요하다

## 상세

Geoffrey Huntley가 만든 "Ralph Wiggum 기법". 2026년 초 AI 코딩 커뮤니티에서 가장 많이 논의된 개발 방법론이다.
